...
interface DepartureResult {
  ...
  departureTimeJaFormatted: string;
  departureSecondsOfDay: number;
  daysOffset: number;
  formattedBulletItem: string;
  formattedBulletJaItem: string;
}
...
interface SavedSettings {
  ...
  meetingPlace: string;
  travelers: PersonTravelInfo[];
  timeFormat?: 'colon' | 'japanese';
  bulletStyle?: 'none' | 'bullet' | 'dash';
  slogan?: string;
}
...
const DEFAULT_7_PEOPLE: PersonTravelInfo[] = [
...
// -------------------------------------------------------------
class AppState {
  targetHours: number = 18;
  targetMinutes: number = 0;
  targetSeconds: number = 0;
  gatheringMinutes: number = 0;
  meetingPlace: string = '';
  travelers: PersonTravelInfo[] = JSON.parse(JSON.stringify(DEFAULT_7_PEOPLE));
  timeFormat: 'colon' | 'japanese' = 'colon';
  bulletStyle: 'none' | 'bullet' | 'dash' = 'none';
  slogan: string = RANDOM_SLOGANS[0];
  sortByEarliest: boolean = true;

  load(): void {
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      if (!raw) return;
      const parsed: Partial<SavedSettings> = JSON.parse(raw);
      if (!parsed) return;
...
// UI更新レンダリング
// -------------------------------------------------------------
function renderTargetTime(): void {
  const hInput = document.getElementById('target-hours') as HTMLInputElement;
  const mInput = document.getElementById('target-minutes') as HTMLInputElement;
  const sInput = document.getElementById('target-seconds') as HTMLInputElement;
...
function renderGatheringTime(): void {
...
function renderMeetingPlace(): void {
  const input = document.getElementById('meeting-place-input') as HTMLInputElement;
  if (input && document.activeElement !== input) {
    input.value = state.meetingPlace;
  }
}

function renderFormatToggles(): void {
  const timeToggles = document.querySelectorAll('[data-time-format]');
  timeToggles.forEach((el) => {
    if ((el as HTMLElement).dataset.timeFormat === state.timeFormat) {
  ...
  bulletToggles.forEach((el) => {
    if ((el as HTMLElement).dataset.bulletStyle === state.bulletStyle) {
      el.classList.add('active');
    } else {
      el.classList.remove('active');
    }
...
function renderTravelersTable(): void {
  for (let i = 0; i < 7; i++) {
    const traveler = state.travelers[i];
    const nameInput = document.getElementById(`traveler-name-${i}`) as HTMLInputElement;
    const minInput = document.getElementById(`traveler-min-${i}`) as HTMLInputElement;
    const secInput = document.getElementById(`traveler-sec-${i}`) as HTMLInputElement;
...
function computeScheduleResults(): {
  scheduleResults: DepartureResult[];
  pureResults: DepartureResult[];
  lines: string[];
  hasPreviousDayOffset: boolean;
} {
  ...
  if (scheduleResults.length > 0) {
    ...
    scheduleResults.forEach((res) => {
      const timeStr =
        state.timeFormat === 'colon' ? res.departureTimeFormatted : res.departureTimeJaFormatted;
...
function renderSchedule(): void {
  ...
  const textarea = document.getElementById('schedule-textarea') as HTMLTextAreaElement;
  ...
  if (scheduleResults.length === 0) {
    ...
    if (lineCountBadge) lineCountBadge.textContent = '0 行';
...
function renderTimeline(pureResults: DepartureResult[]): void {
...
function updateCopyButtonAppearance(): void {
...
// コピー & フラッシュ演出
// -------------------------------------------------------------
function triggerCopyAnimation(): void {
...
async function handleCopySchedule(): Promise<void> {
...
// 一括テキストパース
// -------------------------------------------------------------
function parseBatchText(text: string): PersonTravelInfo[] {
  const lines = text
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean);

  const parsed: PersonTravelInfo[] = [];
...
function setupEventListeners(): void {
  ...
  // 待ち合わせ場所
  const meetingInput = document.getElementById('meeting-place-input') as HTMLInputElement;
  ...
  document.querySelectorAll('[data-time-format]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const fmt = (btn as HTMLElement).dataset.timeFormat as 'colon' | 'japanese';
  ...
  document.querySelectorAll('[data-bullet-style]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const style = (btn as HTMLElement).dataset.bulletStyle as 'none' | 'bullet' | 'dash';
